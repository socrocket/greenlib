...
public:
  ...
  const void* get_value_pointer() const {
    ...
    this->make_post_read_callbacks(); // TODO Note: here the actual read had not yet been done!
    return &m_MemberMap;
  }
...
protected:
  ...
  bool addMember(gs_param_base_T &new_member) {
    ...
    new_member.registerParamCallback(boost::shared_ptr<gs::cnf::ParamCallbAdapt_b>(new gs::cnf::ParamTypedCallbAdapt<gs_param_array_T>(this, &gs_param_array_T::array_config_callback, this, &new_member)), post_read);
