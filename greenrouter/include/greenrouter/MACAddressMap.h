...
typedef std::map <unsigned int, Map_address_t> Ethernet_Map_t;                  ///< address map type
typedef Ethernet_Map_t::iterator Ethernet_Map_iterator_t;         ///< address map iterator type
...
class MACAddressMap {
 ...
 , m_routine_name ("MACAddressMap")             ///< routine name
 ...
 void generateMap (SOCKET& socket) {
   ...
   for ( Port_id_t port_id = 0; port_id < socket.size (); port_id++ ) {
     ...
     GenericRouterBase < TRAITS, PORTMAX, MACAddressMap<TRAITS, PORTMAX> > *            other_router  ( NULL );
     ...
     if (other_router) {
       ...
       // verify port mapping
       for (Ethernet_Map_iterator_t i = m_address_map.begin (); i != m_address_map.end (); i++) {
         if (port_id == i->first) {
           mapped = true;
           break;
         }
 ...
 void dumpMap (bool verbose = GS_VERBOSE_DUMPMAP) {
   if (verbose) {
     ...
     for (Ethernet_Map_iterator_t i = m_address_map.begin (); i != m_address_map.end (); i++)
       cout << "Port id : " << i->first << ", MAC address : " << i->second << endl;
   }
 }

 /** --------------------------------------------------------------------------
  @func decode : parse the ethernet payload. Get the source and destination MAC addresses
  from the first 12 bytes of data field. Return the portId of destination MAC address
  from the m_address_map and update the MAC address of source portId if required.
 ...
 unsigned int from) {               /// portId of the socket from which the trnx is coming
   ...
   //First six bytes contain destination address and following six bytes contain src address
   Map_address_t srcAdd = 0, destAdd = 0;
   memcpy(&destAdd, data, 6);
   memcpy(&srcAdd, data+6, 6);
   //std::cout << std::hex << "destAdd = " << destAdd << ", srcAdd = " << srcAdd << std::dec << "\n";
