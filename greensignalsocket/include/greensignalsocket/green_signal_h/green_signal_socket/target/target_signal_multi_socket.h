...
*/
  ...
  class target_signal_multi_socket : public virtual target_signal_multi_base_socket {
    ...
    public:
      target_signal_multi_socket (const char* name):
      target_signal_multi_base_socket(name) {
        target_signal_multi_base_socket::register_b_transport(this,
                                      &target_signal_multi_socket::b_transport);
        target_signal_multi_base_socket::register_nb_transport_fw(this,
                                  &target_signal_multi_socket::nb_transport_fw);
        this->b_cb = NULL;
        this->nb_cb = NULL;
      }
      ...
      */
      ...
      void b_transport(unsigned int socket_id, transaction_type& trans,
                       sc_core::sc_time& delay) {
        if (!this->b_cb) {
          std::stringstream s;
          s << sc_object::name()
            << ": no blocking transport callback registered";
          SC_REPORT_ERROR("target_signal_multi_socket", s.str().c_str());
        }
        /*
      ...
      sync_enum_type nb_transport_fw(unsigned int socket_id,
