#ifndef TARGET_SIGNAL_MULTI_SOCKET_H_
...
namespace gs_generic_signal {
  typedef gs::socket::target_multi_socket<32,gs_generic_signal_protocol_types,
    gs_generic_signal_bind_checker> target_signal_multi_base_socket;
...
/*!
* @detail The target_signal_multi_socket basically derived from the target_signal_multi_base_socket
...
*/
  ...
  class target_signal_multi_socket : public virtual target_signal_multi_base_socket {
 
    public:
      target_signal_multi_socket (const char* name) : target_signal_multi_base_socket(name) {
        target_signal_multi_base_socket::register_b_transport(this, &target_signal_multi_socket::b_transport);
        target_signal_multi_base_socket::register_nb_transport_fw(this, &target_signal_multi_socket::nb_transport_fw);
      }
      ...
      /*
      //DEPRECATING this as the TEMPLATE version should be used now.
      void set_source (std::string name) { 
        supported_srcid_list.push_back(wrapper_base::decode_name(name));
      }
 
      //DEPRECATING this as by default we listen from all sources. So,
      //user may get confused if none source was set, because this API
      //will return false but we will forward packets from all sources. 
      bool is_src_supported (std::string name) {
        unsigned int max = supported_srcid_list.size();
        unsigned int id = wrapper_base::decode_name(name);

        for (unsigned int i = 0; i<max; ++i) {
          if (supported_srcid_list[i] == id) return true;
        }
        return false;
      }
      */
 
      void register_b_transport (MODULE* mod, void(MODULE::*cb)(unsigned int, transaction_type&, sc_core::sc_time&)) {
        parent_mod = mod;
        b_cb = cb;
      }
 
      void register_nb_transport_fw (MODULE* mod, 
        sync_enum_type(MODULE::*cb)(unsigned int, transaction_type&, phase_type&, sc_core::sc_time&)) {
        parent_mod = mod;
        nb_cb = cb;
      }
 
      void b_transport (unsigned int socket_id, transaction_type& trans, sc_core::sc_time& delay) {
        //check if the source id passed along with the payload is in the supported src list
        if (store_signal_value(trans))
          ((parent_mod->*b_cb)(socket_id, trans, delay));
        //else
          //std::cout << "Source ID of this txn not supported.\n";
        return;
      }
 
      sync_enum_type nb_transport_fw (unsigned int socket_id, transaction_type& trans, phase_type& phase, sc_core::sc_time& delay) {
        //check if the source id passed along with the payload is in the supported src list
        if (store_signal_value(trans))
          return ((parent_mod->*nb_cb)(socket_id, trans, phase, delay));
        //else
          //std::cout << "Source ID of this txn not supported.\n";
    ...
    private:
      ...
      sync_enum_type (MODULE::*nb_cb)(unsigned int, transaction_type&, phase_type&, sc_core::sc_time&) ;
      void (MODULE::*b_cb)(unsigned int, transaction_type&, sc_core::sc_time&) ;
  };
}
#endif
