...
//#define SHOW_SC_TIME(module,msg) cout << "time " << std::setw(3) << sc_time_stamp() << ": " << std::setw(6) << #module << ": " << msg << endl
...
#else
    memcpy(&(t->getSData()[0]), &MEM[t->getMAddr()-target_port.base_addr.value], t->getMBurstLength());
#endif
    return m_delay;
  } else {
#ifdef USE_GPSOCKET
...
#else
    std::cout<<"Got "<<(t->getSData()[0])<<(t->getSData()[1])<<" for address "<<(MAddr)t->getMAddr()<<std::endl;
    memcpy( &MEM[t->getMAddr()-target_port.base_addr.value], &(t->getSData()[0]), t->getMBurstLength());
#endif
    return 2*m_delay;
  }
