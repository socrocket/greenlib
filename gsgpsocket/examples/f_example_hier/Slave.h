...
int SlaveMem::IPmodel( GenericSlaveAccessHandle t)
{
  if (t->getMCmd()==Generic_MCMD_RD){
    ...
    for (unsigned int i=0; i<t->getMBurstLength(); i++) {
      tmp[i] = MEM[i+(t->getMAddr()-target_port.base_addr)];
    } 
     faster:  */
    GSDataType::dtype tmpd(&MEM[(t->getMAddr()-target_port.base_addr)], t->getMBurstLength()); // pesudo-vector without own vector data (but pointer to mem)
...
public:
  ...
  SlaveMemHier(sc_core::sc_module_name name_, int delay_=3) :
    sc_module(name_),
    target_port("tport"),
    child(new SlaveMem((std::string(name_)+"_child").c_str(), delay_))
  {
    // Bind hierarchically
    target_port.bind(child->target_port);
  }
